package com.wutanda.sdk.money.mobilemoney.mtn.openapi.remittances;
...
@Immutable
public final class Remittances {

    private final Environment environment;
    private final String subscriptionKey;

    public Remittances(final Environment environment,final String subscriptionKey){
    ...
    public AccessToken requestAccessToken(final String apiUser, final String apiKey) throws SdkException {
        ...
        final HttpRequest httpRequest = HttpRequest.builder()
                .url(environment.getBaseUrl().concat("/remittance/token/"))
    ...
    public Boolean transfer(final String accessToken, final Transfer transfer) throws SdkException {
        final HttpRequest<RequestToPay> transferHttpRequest = HttpRequest.builder()
                .contentType(ContentType.APPLICATION_JSON)
                .httpMethod(HttpMethod.POST)
                .url(environment.getBaseUrl().concat("/remittance/v1_0/transfer"))
                ...
                .headers(ImmutableMap.<String, String>builder()
                        ...
                        .build())
                .build();
        final HttpResponse<Void> httpResponse = HttpTransport.send(transferHttpRequest, new TypeToken<Void>() {
        }.getType());
    ...
    public Transfer getTransferInfo(final String accessToken, final String paymentReference) throws SdkException {
        final HttpRequest transferHttpRequest = HttpRequest.builder()
                ...
                .url(environment.getBaseUrl().concat("/remittance/v1_0/transfer/").concat(paymentReference))
                .headers(ImmutableMap.<String, String>builder()
                        .put("Ocp-Apim-Subscription-Key", subscriptionKey)
                        .put("X-Target-Environment", environment.name().toLowerCase())
                        .put("Authorization", "Bearer ".concat(accessToken))
                        .build())
                .build();
        final HttpResponse<Transfer> remittanceHttpResponse = HttpTransport.send(transferHttpRequest, new TypeToken<Transfer>() {
        }.getType());
        if (remittanceHttpResponse.getResponseStatusCode() != 200) {
            throw SdkException.internalError("Unable to fetch remittance information");
        }
        return remittanceHttpResponse.getResponseBody();
    }
    ...
    public Balance getAccountBalance(final String accessToken) throws SdkException {
        final HttpRequest<Void> httpRequest = HttpRequest.builder()
                .contentType(ContentType.APPLICATION_JSON)
                .httpMethod(HttpMethod.GET)
                .url(environment.getBaseUrl().concat("/remittance/v1_0/account/balance"))
                .headers(ImmutableMap.<String, String>builder()
                        .put("Ocp-Apim-Subscription-Key", subscriptionKey)
                        .put("X-Target-Environment", environment.name().toLowerCase())
                        .put("Authorization", "Bearer ".concat(accessToken))
    ...
    public AccountHolderInfo getAccountHolderInfo(final String accessToken, final String msisdn) throws SdkException {
        final HttpRequest<Void> httpRequest = HttpRequest.builder()
                .contentType(ContentType.APPLICATION_JSON)
                .httpMethod(HttpMethod.GET)
                .url(environment.getBaseUrl().concat("/remittance/v1_0/accountholder/msisdn/").concat(msisdn).concat("/basicuserinfo"))
                .headers(ImmutableMap.<String, String>builder()
                        .put("Ocp-Apim-Subscription-Key", subscriptionKey)
                        .put("X-Target-Environment", environment.name().toLowerCase())
                        .put("Authorization", "Bearer ".concat(accessToken))
